    ...
    <main>
      <div class="container mx-auto">
        ...
        <div class="lg:columns-2 columns-1">
          <app-budget-card [budgetCardType]="EXPENSE" [icon]="'attach_money'"></app-budget-card>
          <app-budget-card [budgetCardType]="INCOME" [icon]="'trending_up'"></app-budget-card>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 pt-6">
          <ng-container *ngIf="book.expenses | async as expenses">
            ...
            [tableActions]="expenseTableActions">
                  <div class="expense-bg z-10 border rounded my-0 mx-4 p-4 shadow border-b-0 absolute -top-6 left-0 right-0 text-white">
                    <div class="flex flex-row items-center justify-between">
                      <div class="flex flex-col items-center justify-center">
                        <h3 class="text-2xl mb-3">Expenses Table</h3>
                        <p class="text-sm">Add your list of expenses</p>
                      </div>
                      <button mat-icon-button type="button" (click)="addItem(EXPENSE)">
          ...
          <ng-container *ngIf="(book.income | async) as income">            
            <app-table [dataProperties]="budgetItemProperties"
            class="col-span-1"
            [data]="income"
            [pageSizeOptions]="[1, 2, 5, 10]"
            [tableActions]="incomeTableActions">
              <div class="z-10 income-bg border rounded my-0 mx-4 p-4 shadow border-b-0 absolute -top-6 left-0 right-0 text-white">
                  <div class="flex flex-row items-center justify-between">
                    <div class="flex flex-col items-center justify-center">
                      <h3 class="text-2xl mb-3">Income Table</h3>
                      <p class="text-sm">Add your list of income</p>
                    </div>
                    <button mat-icon-button type="button" (click)="addItem(INCOME)">
                      <mat-icon>add</mat-icon>
                    </button>
                  </div>
              </div>
...
export class MainBookComponent implements OnInit {
  ...
  expenseTableActions: TableAction[] = [
    { actionIcon: 'edit', actionCb: (budgetItem: BudgetItem) => this.editItem(budgetItem, this.EXPENSE) },
    { actionIcon: 'delete', actionCb: (budgetItem: BudgetItem) => { this.book.deleteItem(budgetItem, this.EXPENSE) } }
  ]

  incomeTableActions: TableAction[] = [
    { actionIcon: 'edit', actionCb: (budgetItem: BudgetItem) => this.editItem(budgetItem, this.INCOME) },
    { actionIcon: 'delete', actionCb: (budgetItem: BudgetItem) => { this.book.deleteItem(budgetItem, this.INCOME) } }
  ]
